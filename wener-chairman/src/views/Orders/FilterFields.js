  ...
  useEffect(() => {
    setPosts(prevPosts => {
      ...
      if (filters.startDate || filters.endDate) {
        ...
        if (filters.startDate && !filters.endDate) {
          ...
          filteredPosts = filteredPosts.filter(
            ({ create_date }) =>
              create_date.split(",")[0] == formatDate(filters.startDate)
          );
        } else if (!filters.startDate && filters.endDate) {
          //only end
          filteredPosts = filteredPosts.filter(
            ({ create_date }) =>
              create_date.split(",")[0] == formatDate(filters.endDate)
          );
        } else {
          // both
          console.log("both");
      ...
      if (filters.distributor_name) {
        //text
        filteredPosts = filteredPosts.filter(({ distributor_name }) =>
          distributor_name
            .toLowerCase()
            .includes(filters.distributor_name.toLowerCase())
        );
      }
      ...
      if (filters.minAmount || filters.maxAmount) {
        ...
        if (filters.minAmount && !filters.maxAmount) {
          //only start
          filteredPosts = filteredPosts.filter(
            ({ net_total }) => net_total == filters.minAmount
          );
        } else if (!filters.minAmount && filters.maxAmount) {
          //only end
          filteredPosts = filteredPosts.filter(
            ({ net_total }) => net_total == filters.maxAmount
          );
        } else {
          // both
          console.log("both");
  ...
  return (
    <div>
            ...
            >
              <DesktopDatePicker
                ...
                renderInput={params => (
                  <TextField
                    variant="standard"
                    sx={{ marginTop: "5px", transform: "translateX(-14.5px)" }}
                    {...params}
                  />
                )}
              />
      ...
      <TextField
        ...
        autoFocus={focus == "distributor_name"}
        onChange={handleChange}
      ...
      <TextField
        variant="standard"
        name="minAmount"
        sx={{ marginTop: "5px", width: "75px" }}
        autoFocus={focus == "minAmount"}
        onChange={handleChange}
      ...
      <IconButton
        onClick={reset}
        sx={{ padding: "0", transform: "translate(120px,9.5px)" }}
        aria-label="delete"
      >
        {/* <RefreshIcon /> */}
      </IconButton>
