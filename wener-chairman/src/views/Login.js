...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    if (error) {
      ...
      if (profile.data.length > 0 && profile.body[0].role === 'chairman') {
        ...
        supabase.auth.signOut().then((data) => {
          setErrorMessage("This dashboard is for Chairman users only")
          console.log(data);
        })
      }
    }
          ...
          <Card small>
            ...
            <ListGroup flush>
              <ListGroupItem className="px-3">
                {errorMessage ? <p style={{ marginBottom: '0px', color: 'red', textAlign: 'right' }}>{errorMessage}</p> : null}
                <Row form>
