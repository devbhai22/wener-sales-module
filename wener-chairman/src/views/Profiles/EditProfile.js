            ...
            if (role === 'zsm') {
                if (dm !== '' && location !== '') {
                    const { profile, error2 } = await supabase
                        .from('profiles')
                        .update({ name: name, works_under: dm, works_at: location, age: age, avatar_url: profilePicture, attachment_path: attachmentPath })
                        .eq('id', user.id)
                    if (error2) {
                        console.log(error2)
                    }
                ...
                if (zm !== '' && dm !== '' && location !== '') {
                    const { profile, error3 } = await supabase
                        .from('profiles')
                        .update({ name: name, works_under: zm, works_at: location, age: age, avatar_url: profilePicture, attachment_path: attachmentPath })
                        .eq('id', user.id)
                    if (error3) {
                        console.log(error3)
                    }
                ...
                if (location !== '') {
                    const { profile, error4 } = await supabase
                        .from('profiles')
                        .update({ name: name, works_at: location, age: age, avatar_url: profilePicture, attachment_path: attachmentPath })
                        .eq('id', user.id)
                    if (error4) {
                        console.log(error4)
                    }
                ...
                const { profile, error5 } = await supabase
                    .from('profiles')
                    .update({ name: name, age: age, avatar_url: profilePicture })
                    .eq('id', user.id)
                if (error5) {
                    console.log(error5)
                }
