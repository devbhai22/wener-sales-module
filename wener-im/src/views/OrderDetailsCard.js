...
const OrderDetailsCard = ({ id }) => {
    ...
    async function approveOrder(e) {
        ...
        const { data, error } = await supabase
            .from('order_events')
            .insert([
                { name: 'OrderApprovedByIM', created_by: user.id, 'data': eventData },
            ])
        if (error) {
            console.log(error);
        } else {
    ...
    async function approveReturn(e) {
        ...
        const { data, error } = await supabase
            .from('order_events')
            .insert([
                { name: 'ReturnApprovedByIM', created_by: user.id, 'data': eventData },
            ])
        if (error) {
            console.log(error);
        } else {
    ...
    async function rejectOrder(e) {
        ...
        const { data, error } = await supabase
            .from('order_events')
            .insert([
                { name: 'OrderRejectedByIM', created_by: user.id, 'data': eventData },
            ])
        if (error) {
            console.log(error);
        } else {
            ...
            <ListGroupItem className="p-0 m-0 mx-auto">
                ...
                <ListGroupItem className="px-4 pb-5 pb-0">
                    <strong className="text-muted d-block mt-1">
                        <span style={{ color: '#525252' }}>Created On : </span>
                        {order.invoice_data? order.timestamp.split('T')[0] : null}</strong>
                    <strong className="text-muted d-block mt-1">
                ...
                {order.status === 'Return Approved By DM' ? (
                    <>
                        <ListGroupItem className="p-0 mx-auto">
                            <button
                                className="btn btn-secondary"
